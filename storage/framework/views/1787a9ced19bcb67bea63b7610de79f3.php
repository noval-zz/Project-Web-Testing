<?php $__env->startSection('title', 'Pantau Laporan — Sistem Pelaporan Fasilitas'); ?>
...
<?php
  ...
  $stepMap = function(string $status): int {
    if (str_contains($status, 'Selesai'))    return 4;
    if (str_contains($status, 'Diperbaiki')) return 3;
    if (str_contains($status, 'Diproses'))   return 2;
    return 1; // Menunggu / default
  };
  ...
  $statusClass = function(string $s): string {
    if (str_contains($s, 'Selesai'))    return 'badge-success';
    if (str_contains($s, 'Diperbaiki')) return 'badge-primary';
    return 'badge-warning';
  };

  $miniBadgeClass = function(string $s): string {
    if (str_contains($s, 'Selesai'))    return 'mb-selesai';
    if (str_contains($s, 'Diperbaiki')) return 'mb-proses';
    return 'mb-default';
  };
?>
        ...
        <div class="panel-list-body" id="report-list">
            <?php if($laporan->isEmpty()): ?>
                <div class="list-empty">
                    ...
                    <a href="<?php echo e(route('laporan.create')); ?>">➕ Buat Laporan</a>
                </div>
            <?php else: ?>
                ...
                ?>
                <div class="report-item <?php echo e($isFirst ? 'active' : ''); ?>"
                     data-id="<?php echo e($lap->id_laporan); ?>"
                     data-status="<?php echo e(strtolower($status)); ?>"
                     data-tingkat="<?php echo e(strtolower($tingkat)); ?>"
                     data-search="<?php echo e(e(strtolower($ticketNum . ' ' . $kategori . ' ' . $lokasi . ' ' . $tingkat))); ?>">
                    <div class="item-top">
                        <span class="item-ticket"><?php echo e($ticketNum); ?></span>
                        <span class="mini-badge <?php echo e($miniBadgeClass($status)); ?>">
                            <?php echo e(str_contains($status,'Selesai') ? 'Selesai' : (str_contains($status,'Diperbaiki') ? 'Diproses' : 'Menunggu')); ?>

                        </span>
                    </div>
                    <div class="item-title"><?php echo e($kategori); ?></div>
                    <div class="item-sub">
                        <span>📍 <?php echo e($lokasi); ?></span>
                        <span>· <?php echo e($lap->created_at->format('d M Y')); ?></span>
                    </div>
                </div>
...
<?php
    ...
    $_stepLabels = [
        1 => 'Laporan Diterima',
        2 => 'Sedang Diverifikasi Admin',
        3 => 'Dalam Pengerjaan Teknisi',
        4 => 'Perbaikan Selesai',
    ];
    $_stepNotes = [
        1 => 'Laporan berhasil dikirim ke sistem dan masuk antrian verifikasi admin Sarpras.',
        2 => 'Admin Sarpras sedang meninjau dan memverifikasi laporan Anda.',
        3 => 'Teknisi telah ditugaskan dan sedang melakukan perbaikan di lokasi.',
    ...
    foreach ($laporan as $lap) {
        ...
        $_kat  = ucfirst($lap->kategori->nama_kategori ?? 'Fasilitas');
        $_lok  = $lap->lokasi
            ? $lap->lokasi->nama_ruangan . ', ' . $lap->lokasi->nama_gedung
            : 'Lokasi tidak diketahui';
        $_foto = $lap->foto ? asset('storage/' . $lap->foto) : null;
...
?>
<script>
const reportsData = <?php echo json_encode($reportsJson, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT); ?>;
...
function getStatusClass(status) {
    if (status.includes('Selesai'))    return 'badge-success';
    if (status.includes('Diperbaiki')) return 'badge-primary';
    return 'badge-warning';
}
...
function applyFilter(query = null) {
    ...
    document.querySelectorAll('.report-item').forEach(el => {
        ...
        let showByPill = true;
        if (currentFilter === 'proses')   showByPill = status.includes('diperbaiki') || status.includes('menunggu');
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\laravel - Copy (2)\resources\views/mahasiswa/pantau_laporan.blade.php ENDPATH**/ ?>