...
@section('page-title','Edit Kategori Fasilitas')

@section('content')
<div style="max-width:560px">
    <div class="card">
        <div class="card-header">
            <div class="card-title">Edit: {{ $kategori->nama_kategori }}</div>
            <a href="{{ route('superadmin.kategori.index') }}" class="btn btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
        </div>
        <form method="POST" action="{{ route('superadmin.kategori.update', $kategori->id_kategori) }}">
            @csrf @method('PUT')
