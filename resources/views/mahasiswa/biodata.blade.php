    ...
    <div class="col-lg-9">
      ...
      <div class="card card-custom bg-white">
        ...
        <div class="card-header card-header-futuristic py-4 px-4 d-flex justify-content-between align-items-center">
          <div>
            <h4 class="mb-1 fw-bold text-white" style="letter-spacing: -0.5px;">Edit Data {{ ucfirst($user->role === 'dosen' ? 'Dosen' : 'Mahasiswa') }}</h4>
            <p class="mb-0 text-white-50 small">Pusat pembaruan enkripsi data dan profil mahasiwa.</p>
          </div>
          <a href="{{ route('mahasiswa.dashboard') }}" class="btn btn-glass btn-sm fw-medium">
        ...
        <div class="card-body p-4 p-md-5">
          ...
          <form action="{{ route('mahasiswa.biodata.update', $mhs->id_mahasiswa) }}"
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-4">
                <label class="form-label">{{ $user->role === 'dosen' ? 'NIDN/NIP (Nomor Induk Dosen)' : 'NIM (Nomor Induk Mahasiswa)' }}</label>
                <input type="number"
                       class="form-control"
                       name="Nim"
                       required
                ...
                <div class="custom-selection-box d-flex flex-wrap gap-3 align-items-center" style="min-height: calc(2.25rem + 14px);">
                  @foreach (['Seni', 'olahraga', 'Robotika'] as $u)
                    <div class="form-check m-0 ms-2">
                      <input class="form-check-input" type="checkbox" name="ukm[]" id="ukm_{{ $u }}" value="{{ $u }}"
                        {{ in_array($u, $ukm_array) ? 'checked' : '' }}>
                      <label class="form-check-label" for="ukm_{{ $u }}">{{ ucfirst($u) }}</label>
                    </div>
                  @endforeach
                </div>
              </div>
              ...
              <div class="col-12 mb-4">
                ...
                <div class="d-flex align-items-center gap-3 avatar-container">
                  @if ($mhs->foto_profil)
                    <img src="{{ asset('storage/' . $mhs->foto_profil) }}" class="avatar-preview" alt="Foto profil">
                  @else
