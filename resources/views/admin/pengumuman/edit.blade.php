...
@section('title', 'Edit Pengumuman Sarpras')
...
<div class="form-wrap">
  ...
  <div class="form-card">
    <h2><i class="fa-solid fa-pen-to-square"></i> Edit Pengumuman Sarpras</h2>
    ...
    <form action="{{ route('admin.pengumuman.update', $pengumuman->id_pengumuman) }}" method="POST">
      ...
      <div class="form-group">
        ...
        <input type="text" id="judul" name="judul" placeholder="Contoh: Perbaikan Jaringan Internet Kampus"
               value="{{ old('judul', $pengumuman->judul) }}" maxlength="200">
      ...
      <div class="form-group">
        ...
        <textarea id="isi" name="isi" placeholder="Tulis isi pengumuman secara detail...">{{ old('isi', $pengumuman->isi) }}</textarea>
      ...
      <div class="form-group">
        ...
        <select id="status" name="status">
          <option value="aktif"    {{ old('status', $pengumuman->status) === 'aktif' ? 'selected' : '' }}>✅ Aktif — Tampil di dashboard mahasiswa</option>
          <option value="nonaktif" {{ old('status', $pengumuman->status) === 'nonaktif' ? 'selected' : '' }}>⛔ Nonaktif — Disembunyikan dari mahasiswa</option>
        </select>
        @error('status')<div class="error-msg">{{ $message }}</div>@enderror
      </div>

      <div class="form-actions">
        <button type="submit" class="btn-submit" id="btn-update-pengumuman">
